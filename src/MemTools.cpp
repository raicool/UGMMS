...
namespace MemTools 
{
	...
	const void* scan_memory(void* address_low, std::size_t nbytes, const std::vector<uint8_t>& bytes_to_find) 
	{
	...
	void* scanLocal(const std::vector<uint8_t>& bytes_to_find, void* begin, int count)
	{
		...
		const uint8_t* found = std::search((uint8_t*)begin, end, bytes_to_find.begin(), bytes_to_find.end(), [](char fromText, char fromPattern)
		{
			return fromPattern == '?' || fromPattern == fromText;
		});
	...
	void* readPointer(void* start)
	{
		...
		for (int i = 0; i < 4; i++) 
		{
			address += (uint64_t)(*((uint8_t*)start + i)) * (uint64_t)pow(16, i * 2);
		}
		return address;
	}
